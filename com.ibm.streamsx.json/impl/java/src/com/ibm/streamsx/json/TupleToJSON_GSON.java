...
import com.ibm.streamsx.json.converters.TupleToJSONConverter_GSON;
...
@PrimitiveOperator(name="TupleToJSON_GSON", description=TupleToJSON_GSON.DESC)
@Libraries("lib/*")
public class TupleToJSON_GSON extends AbstractOperator {
	...
	private static Logger l = Logger.getLogger(TupleToJSON_GSON.class.getCanonicalName());
	...
	public void process(StreamingInput<Tuple> stream, Tuple tuple) throws Exception 	{
		...
		if(rootAttribute == null) 
			jsonData = TupleToJSONConverter_GSON.convertTuple(tuple);
		else {
			if(rootAttributeType.getMetaType() == MetaType.TUPLE)
				jsonData = TupleToJSONConverter_GSON.convertTuple(tuple.getTuple(rootAttribute));
			else 
				jsonData = TupleToJSONConverter_GSON.convertArray(tuple, rootAttribute);
		}
